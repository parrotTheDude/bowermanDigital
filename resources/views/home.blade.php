...
<section class="relative overflow-hidden bg-cream">
  ...
  <div class="relative mx-auto max-w-7xl px-6 pt-28 pb-16 text-center md:pl-24 md:pt-36 md:pb-20">
    <p class="font-mono text-[13px] uppercase tracking-[0.2em] text-rust-deep opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="0">Sydney &middot; Australia</p>

    <h1 class="mx-auto mt-3 max-w-3xl font-display text-4xl font-black leading-[1.06] tracking-tight md:text-6xl opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="50">
    ...
    <div class="mt-7 flex flex-wrap justify-center gap-4 opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="150">
...
<div class="marquee-retro border-y-2 border-ink bg-ink py-3.5 text-cream" aria-hidden="true">
  <div class="marquee-track font-mono text-[13px] uppercase tracking-[0.16em]">
    @php $items = ['AI search optimisation','websites that load before you blink','llms.txt (ask us)','automation for the boring bits','no lock-in contracts','yes, a real person']; @endphp
...
{{-- ─── Work ─── --}}
<section class="border-y-2 border-ink bg-cream-2 px-6 py-12 md:py-16">
  <div class="mx-auto max-w-7xl">
      ...
      <a href="{{ url('/portfolio/crystal-caves') }}" class="grid no-underline md:grid-cols-2">
        ...
        <div class="flex flex-col justify-center p-7 md:p-10">
          <div class="flex flex-wrap gap-2">
            <span class="rounded-full border-[1.5px] border-ink bg-cream-3 px-2.5 py-0.5 font-mono text-[11.5px]">Website Rebuild</span>
            <span class="rounded-full border-[1.5px] border-ink bg-cream-3 px-2.5 py-0.5 font-mono text-[11.5px]">SEO &amp; AI Search</span>
            <span class="rounded-full border-[1.5px] border-ink bg-cream-3 px-2.5 py-0.5 font-mono text-[11.5px]">Tourism</span>
          </div>
    ...
    <div class="mt-5 grid gap-5 md:grid-cols-3">
      @php
        $projects = [
          ['href' => '/portfolio/vizzbud', 'img' => 'images/vizzbud/vizzbud-feature', 'w' => 992, 'alt' => 'Vizzbud dive conditions platform', 'tags' => ['iOS','Android','Web'], 'h' => 'Vizzbud', 'p' => 'Our own project: realtime dive conditions and a logbook for divers, on web, iOS and Android.'],
      ...
      @foreach ($projects as $i => $pr)
        <article class="sticker {{ $i % 2 ? 'tilt-r' : 'tilt-l' }} overflow-hidden bg-cream opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="{{ 150 + $i*50 }}">
          <a href="{{ url($pr['href']) }}" class="flex h-full flex-col no-underline">
            ...
            <div class="flex flex-1 flex-col p-6">
              <div class="mb-3 flex flex-wrap gap-2">
                @foreach ($pr['tags'] as $tag)
                  <span class="rounded-full border-[1.5px] border-ink bg-cream-3 px-2.5 py-0.5 font-mono text-[11.5px]">{{ $tag }}</span>
                @endforeach
              </div>
              <h3 class="font-display text-lg font-semibold">{{ $pr['h'] }}</h3>
              <p class="mt-1.5 text-base text-ink-soft">{{ $pr['p'] }}</p>
              <span class="mt-auto pt-5 inline-block font-mono text-sm font-bold text-rust-deep">View case study →</span>
            </div>
          </a>
        </article>
      @endforeach
...
<section class="bg-cream px-6 py-12 md:py-16">
  <div class="mx-auto max-w-7xl">
    <div class="mb-8 flex flex-wrap items-end justify-between gap-6">
      ...
      <p class="max-w-sm text-ink-soft opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="50">What we actually do all day.</p>
    </div>
    ...
    <div class="grid gap-6 md:grid-cols-2">
      @php
        $services = [
          ...
          ['n' => 'No. 03', 'bg' => 'bg-[#e9e3c6]', 'h' => 'CRMs & automation',        'p' => "Custom CRMs built around how you actually work. One client runs participants, staff and bookings on a system we built.", 'd' => 'bookings · staff · follow-ups · reporting'],
    ...
    <div class="mt-8 flex justify-center opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="150">
      <a href="{{ url('/services') }}" class="btn-retro">Everything we do →</a>
    </div>
  </div>
</section>
...
<section class="border-y-2 border-ink bg-cream-2 px-6 py-20 md:py-28">
  <div class="mx-auto max-w-7xl">
    <div class="mb-14 flex flex-wrap items-end justify-between gap-6">
      ...
      <p class="max-w-sm text-ink-soft opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="50">You deal with us the whole way through. No hand-offs, no getting passed around.</p>
    </div>

    <div class="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
      @php
        $steps = [
          ['n' => '01', 'bg' => 'bg-cream',     'h' => 'You get in touch', 'p' => "Tell us what's not working. You'll hear back from us, usually the same day.", 'g' => 'a real reply, fast'],
          ...
          ['n' => '03', 'bg' => 'bg-[#eed9b2]', 'h' => 'A free proposal',   'p' => "We put together a clear proposal — what we'll do, what it costs, how long it takes — in plain English. No obligation.", 'g' => 'a clear proposal, free'],
          ['n' => '04', 'bg' => 'bg-[#e9e3c6]', 'h' => 'We build it',       'p' => "If you're happy, we kick off. We keep you in the loop the whole way through, until it's finished and handed over.", 'g' => 'updates the whole way'],
        ];
      @endphp
      @foreach ($steps as $i => $st)
        <article class="sticker {{ $i % 2 ? 'tilt-r' : 'tilt-l' }} {{ $st['bg'] }} flex flex-col p-7 opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="{{ 100 + $i*50 }}">
          ...
          <p class="text-base text-ink-soft">{{ $st['p'] }}</p>
...
<section class="border-t-2 border-ink bg-rust px-6 py-24 text-center md:py-32">
  <div class="mx-auto max-w-3xl">
    ...
    <p class="mx-auto mt-5 mb-10 max-w-lg text-lg text-[#fff7ea]/90 opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="50">
      Tell us about your business and what's not working. We'll come back to you the same day with a plan, in plain English and no obligation.
    </p>
