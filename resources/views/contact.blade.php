...
<section class="relative bg-cream">
  <div class="relative z-10 mx-auto max-w-7xl px-6 pt-24 pb-16 md:pt-40">
    ...
    <div class="mx-auto max-w-3xl text-center">
      <p class="font-mono text-[13px] uppercase tracking-[0.2em] text-rust-deep opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="0">Let's talk</p>
      <h1 class="mt-4 font-display text-4xl font-black md:text-5xl opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="50">Tell us about your project</h1>
      <p class="mt-4 text-ink-soft opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="100">We usually reply within one business day. Share a few details and we'll get back with next steps.</p>
    </div>

    {{-- Success banner --}}
    @if(session('status'))
      <div class="mx-auto mt-8 max-w-2xl rounded-xl border border-green-700/40 bg-green-100 px-5 py-4 text-green-800">
        {{ session('status') }}
      </div>
    @endif
    ...
    {{-- Error summary --}}
    @if($errors->any())
      <div class="mx-auto mt-8 max-w-2xl rounded-xl border border-red-700/40 bg-red-100 px-5 py-4">
        <p class="font-medium text-red-800">Please fix the errors below and resubmit.</p>
    ...
    <div class="mx-auto mt-6 max-w-5xl grid grid-cols-1 gap-10 lg:grid-cols-[1fr_320px] lg:items-start">
      ...
      <div class="relative opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="150">
        <div class="relative rounded-2xl border-2 border-ink bg-cream-2 p-6 md:p-8">
          <form id="contact-form" action="{{ route('contact.submit') }}" method="POST" novalidate>
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
              ...
              <div>
                <label for="name" class="block text-xs uppercase tracking-wide text-ink-soft font-medium">Name *</label>
                <input
                  id="name"
                  name="name"
                  type="text"
                  required
                  value="{{ old('name') }}"
                  class="mt-2 w-full rounded-xl border-2 {{ $hasNameErr ? 'border-red-700/60' : 'border-ink' }} bg-white/60 px-4 py-3 text-ink placeholder-ink-faint focus:outline-none focus:ring-2 focus:ring-rust/60 transition"
                  placeholder="Name"
                  @if($hasNameErr) aria-invalid="true" aria-describedby="name-error" @endif
                >
                @error('name')
                  <p id="name-error" class="mt-2 text-sm text-red-800">{{ $message }}</p>
                @enderror
              </div>
              ...
              <div>
                <label for="email" class="block text-xs uppercase tracking-wide text-ink-soft font-medium">Email *</label>
                <input
                  id="email"
                  name="email"
                  type="email"
                  required
                  value="{{ old('email') }}"
                  class="mt-2 w-full rounded-xl border-2 {{ $hasEmailErr ? 'border-red-700/60' : 'border-ink' }} bg-white/60 px-4 py-3 text-ink placeholder-ink-faint focus:outline-none focus:ring-2 focus:ring-rust/60 transition"
                  placeholder="you@company.com"
                  @if($hasEmailErr) aria-invalid="true" aria-describedby="email-error" @endif
                >
                @error('email')
                  <p id="email-error" class="mt-2 text-sm text-red-800">{{ $message }}</p>
                @enderror
              </div>
              ...
              <div class="md:col-span-2">
                <label for="phone" class="block text-xs uppercase tracking-wide text-ink-soft font-medium">Phone <span class="normal-case text-ink-faint">(optional)</span></label>
                <input
                  id="phone"
                  name="phone"
                  type="tel"
                  value="{{ old('phone') }}"
                  class="mt-2 w-full rounded-xl border-2 {{ $hasPhoneErr ? 'border-red-700/60' : 'border-ink' }} bg-white/60 px-4 py-3 text-ink placeholder-ink-faint focus:outline-none focus:ring-2 focus:ring-rust/60 transition"
                  placeholder="+61 ..."
                  @if($hasPhoneErr) aria-invalid="true" aria-describedby="phone-error" @endif
                >
                @error('phone')
                  <p id="phone-error" class="mt-2 text-sm text-red-800">{{ $message }}</p>
                @enderror
              </div>
            </div>
            ...
            <div class="mt-5">
              <label for="message" class="block text-xs uppercase tracking-wide text-ink-soft font-medium">Message *</label>
              <textarea
                id="message"
                name="message"
                rows="5"
                required
                class="mt-2 w-full rounded-xl border-2 {{ $hasMsgErr ? 'border-red-700/60' : 'border-ink' }} bg-white/60 px-4 py-3 text-ink placeholder-ink-faint focus:outline-none focus:ring-2 focus:ring-rust/60 transition"
                placeholder="Tell us about your project…"
                @if($hasMsgErr) aria-invalid="true" aria-describedby="message-error" @endif
              >{{ old('message') }}</textarea>
              @error('message')
                <p id="message-error" class="mt-2 text-sm text-red-800">{{ $message }}</p>
              @enderror
            </div>
            ...
            <div class="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
              <button
                type="submit"
                id="submitBtn"
                class="btn-retro btn-retro-solid !w-full sm:w-auto relative inline-flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
                aria-live="polite"
                aria-busy="false"
                data-loading="false"
              >
      ...
      <div class="flex flex-col gap-5 lg:sticky lg:top-28">
        ...
        {{-- What happens next --}}
        <div class="rounded-2xl border-2 border-ink bg-cream-3 p-5 opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="200">
          <h2 class="font-display text-sm font-black text-ink">What happens next</h2>
        ...
        {{-- Quick facts --}}
        <div class="rounded-2xl border-2 border-ink bg-cream-3 p-5 opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="250">
          <h2 class="font-display text-sm font-black text-ink">Why Bowerman Digital</h2>
...
{{-- ─── Direct Contact ─── --}}
<section class="relative border-t-2 border-ink bg-cream-2">
  <div class="relative z-10 mx-auto max-w-5xl px-6 pt-8 pb-10 md:pt-10 md:pb-14">
    <div class="grid grid-cols-1 gap-10 md:grid-cols-3">
      ...
      <div class="text-center opacity-0 translate-y-6 transition-all duration-700 ease-out reveal" data-delay="0">
        <div class="mx-auto flex h-12 w-12 items-center justify-center rounded-xl border-2 border-ink bg-mustard">
        ...
        <h2 class="mt-3 font-display text-sm font-black text-ink">Email us</h2>
        <a href="mailto:hello@bowermandigital.com" class="mt-1 block font-mono text-sm font-bold text-rust-deep transition-colors no-underline">hello@bowermandigital.com</a>
      </div>
    ...
    (function () {
      ...
      function showInlineError(msg) {
        ...
        if (!el) {
          ...
          el.className = 'mt-3 rounded-xl border border-red-700/40 bg-red-100 px-4 py-3 text-sm text-red-800';
